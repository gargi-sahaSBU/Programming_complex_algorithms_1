...
def paired_t_test(X, Y, K, alpha):
    ...
    for i in range(K):
        ...
        num_err_full = 0
        for j in range(0,len(Y_minus_Yi)):
            if Y_minus_Yi[j] != pred_Y[j]:
                num_err_full = num_err_full + 1
        num_err_naive = 0
        for j in range(0,len(Y_minus_Yi)):
            if Y_minus_Yi[j] != pred_naive[j]:
                num_err_naive = num_err_naive + 1
    ...
    variance = 0
    for d in diff:
        variance = variance + ((d - mean_diff) * (d - mean_diff))
    variance = variance/K
